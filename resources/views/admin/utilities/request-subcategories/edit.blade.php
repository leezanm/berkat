...
<div class="container">
    <div class="row mb-4">
        <div class="col-md-8">
            <h3><i class="fas fa-edit"></i> Ubah Sub-Kategori Bantuan</h3>
        </div>
    </div>

    <div class="row">
        <div class="col-md-8">
            <form action="{{ route('admin.request-subcategories.update', $requestSubcategory) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('request_category_id') is-invalid @enderror" id="request_category_id" name="request_category_id" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('request_category_id', $requestSubcategory->request_category_id) == $category->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $requestSubcategory->name) }}" required>
                            @error('name') <span class="invalid-feedback d-block">{{ $message }}</span> @enderror
                        </div>

                        <div class="mb-3">
                            <label for="description" class="form-label">Huraian</label>
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4">{{ old('description', $requestSubcategory->description) }}</textarea>
                            @error('description') <span class="invalid-feedback d-block">{{ $message }}</span> @enderror
                        </div>

                        <div class="mb-3">
                            <label for="amount" class="form-label"><i class="fas fa-money-bill"></i> Amaun Bantuan (RM)</label>
                            <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', $requestSubcategory->amount) }}" step="0.01" min="0">
